        ...
        <transition name="drop-in">
          <div class="modal-inner" v-show="isOpen" ref="target" >
            ...
            <div class="mb-8 overflow-y-scroll h-80 text-[8px] sm:text-base">
              ...
              <span v-for="device in fastWateringList" class="grid grid-cols-2 text-start text-white mb-2 border-b border-white">
                <p class="col-span-1 text-center">{{ device.device_name }}</p>
                <div class="w-full flex justify-center items-center mb-3">
                  <label class="inline-flex relative items-center cursor-pointer">
                    <input 
                    type="checkbox"
                    v-model="device.S988"
                    @change="onToggle(device.device,device.S988)"
                    class="sr-only peer">
