<template>
  ...
  <alert message="Idrosat coordinate not appropriate" :modalActive="!isIdrosatCoordinateValid" :isError="true"
    @close="closeNotification" />
  <RepositionIdrosat :id="props.id" :isOpen="repositionIdrosatModalActive" @close="idrosatModalToggle"
    :title="$t('changeIdrosatCoordinate')" :latLng="updatedIdrosatCoordinate" @coordinatesChanged="idrosatUpdated" />
  <ConfigDispenser :isOpen="configureDispenserModalActive" @close="dispenserModalToggle" :title="$t('addEV')"
    :evList="availableEV" :deviceData="devicesStore.deviceData" :formData="devicesStore.deviceGeo"
    :latLng="clickedLatLng" @coordinatesChanged="dispenserUpdated" />
  <div class="map-container">
    <sidebar :noSocial="true" :backOn="true" />
    <div class="device-container">
      <deviceCard :small=true :content="newData" :id="props.id" />
      ...
      <div class="main">
        ...
        <div class="button-wrapper">
          <div v-if="!changeDispenserCoordinate" class="flex flex-col gap-2 justify-center items-center mt-1 sm:mt-4">
            <IveButton v-if="!changeIdrosatCoordinate" type="button" class="filled__blue mt-1 sm:mt-6"
              :label="$t('configureIdrosat')" @click="enableIdrosatConfig" />
          ...
          <div v-if="!changeIdrosatCoordinate" class="flex flex-col gap-2 justify-center items-center mt-1 sm:mt-4">
            <IveButton v-if="!changeDispenserCoordinate" type="button" class="filled__blue"
              :label="$t('configureDispenser')" @click="enableDispenserConfig" />
            <h2 class="font-semibold" v-if="changeDispenserCoordinate">{{ $t('clickOnMapDispenser') }}</h2>
            <IveButton v-if="changeDispenserCoordinate" type="button" class="filled__red" :label="$t('cancel')"
              @click="disableDispenserConfig" />
          </div>
        </div>
      </div>
    </div>
  ...
  for (const data of evMarkerPosition.value) {
    ...
    const infoWindow = new google.maps.InfoWindow({
      content: `<div style="display:flex;flex-direction: column; gap:4px;">
        <p>Station Number: <span style="font-weight: 600;">${data.stationNumber}</span></p>
        <p>Station Serial: <span style="font-weight: 600;">${data.serial}</span></p>
        </div>`, // HTML content for the info window
    })
...
async function getEvGeo() {
  ...
  for (let i = 0; i < dataStore.evConfigLength / 5; i++) {
    if (dataStore.evConfig.hasOwnProperty('S' + EVCONFIG_START_ADDRESS)) {
      if (dataStore.evConfig['S' + EVCONFIG_START_ADDRESS] !== "FFFFFF") {
        let mainDataObj = {
          id: String(index++),
          serial: dataStore.evConfig === undefined ? undefined : dataStore.evConfig['S' + EVCONFIG_START_ADDRESS],
          stationNumber: dataStore.evConfig === undefined ? undefined : dataStore.evConfig['S' + (EVCONFIG_START_ADDRESS + 2)],
        ...
        availableStation.value.push(mainDataObj)
      }
    }
  ...
  //merging available station with station geo data
  devicesStore.deviceGeo.forEach(device => {
    const match = availableStation.value.find(pump => pump.serial === device.ev_serial);
    if (match) {
      device.stationNumber = match.stationNumber;
    }
  })

  let activeStationsList = dataStore.satStat === undefined ? 0 : dataStore.satStat.S18?.split(',')

  devicesStore.deviceGeo.map((station) => {
    // let activeStation = dataStore.satStat['S' + (stationStatusFirstAddress + index)] === undefined ? undefined : dataStore.satStat['S' + (stationStatusFirstAddress + index)].split(',')

    //check is station acive or not
    let activeStation
    if (activeStationsList !== undefined) {
      activeStation = activeStationsList.includes(station.stationNumber) ? '1' : '0'
    }
    ...
    if (activeStation !== undefined) {
      newObj.stationNumber = station.stationNumber
      newObj.serial = station.ev_serial
      newObj.status = activeStation
      newObj.lat = parseFloat(station.latitude)
      newObj.lng = parseFloat(station.longitude)
    }
...
<style scoped>
.background {
  @apply bg-[#99D2E2] fixed h-screen w-screen z-0
}
...
.map-style {
  @apply h-[280px] sm:h-[455px] md:h-[537px] lg:h-[527px] xl:h-[605px] 2xl:h-[660px] w-[100%]
  /* w-[320px] 
    sm:w-[400px] 
    lg:w-[471px] 
    xl:w-[656px] 
